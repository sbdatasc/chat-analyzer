...
/// Wipe every derived node (topic / concept / entity / source / pattern)
/// along with its edges, embeddings, FTS rows, and parked proposals, then
/// re-queue every conversation for fresh extraction. The ingested
/// conversations.json store and `message_index` are untouched, so the user
/// never needs to re-import. All work happens in a single transaction.
#[command]
async fn rebuild_knowledge_graph(
    ...
    // 1. Wipe every edge that touches a derived node. Edges between two
    //    conversations don't exist in the current schema, so this is
    //    effectively all non-conversation-adjacent edges.
    ...
    // 2. Drop the derived nodes' vector and FTS rows before the node delete —
    //    those are virtual tables without FK cascade.
    tx.execute(
    ...
    // 3. Delete the derived nodes.
    let derived_nodes_deleted = tx
        .execute("DELETE FROM node WHERE type != 'conversation'", [])
        .map_err(|e| e.to_string())? as i64;

    // 4. Clear parked items — they're per-conversation proposals that must
    //    be regenerated by the fresh run.
    let parked_deleted = tx
        .execute("DELETE FROM parked_extractions", [])
        .map_err(|e| e.to_string())? as i64;

    // 5. Requeue every conversation. Rows without an extraction_state row
    //    get inserted as pending so nothing is missed.
    let updated = tx
        .execute(
            "UPDATE extraction_state
